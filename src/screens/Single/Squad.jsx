...
const SingleSquadWorkoutScreen = ({ navigation }) => {
  const [level, setLevel] = useState(1);
  const [exerciseIndex, setExerciseIndex] = useState(0);
  const [currentSet, setCurrentSet] = useState(1);
  const [restTime, setRestTime] = useState(0);
  const [isResting, setIsResting] = useState(false);
  const [exercise] = useState(ExerciseService.getSingleSquad());
  ...
  const getDataFromAsyncStorage = async () => {
    try {
      const storedStatus = await AsyncStorage.getItem('@singleSquadWorkoutStatus');
      if (storedStatus === null) {
        await AsyncStorage.setItem('@singleSquadWorkoutStatus', JSON.stringify({}));
      } else {
        const singleSquadWorkoutStatusData = JSON.parse(storedStatus);
        const currentLevel = singleSquadWorkoutStatusData['SingleSquadWorkout']?.level || 1;
        setLevel(currentLevel);
      }

      const savedCurrentSet = await AsyncStorage.getItem('@singleSquadCurrentSet');
  ...
  const storeData = async () => {
    try {
      await AsyncStorage.setItem('@singleSquadWorkoutStatus', JSON.stringify({
        'SingleSquadWorkout': { level }
      }));
      await AsyncStorage.setItem('@singleSquadCurrentSet', JSON.stringify(currentSet));
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleCompleteSet = async () => {
    ...
    const completedExerciseStats = singleSquadWorkout[exercise.name] || { completedCount: 0, totalReps: 0 };
    completedExerciseStats.completedCount += 1;
    completedExerciseStats.totalReps += repsInThisSet;

    singleSquadWorkout[exercise.name] = completedExerciseStats;

    try {
      await AsyncStorage.setItem('@singleSquadWorkoutStatus', JSON.stringify(singleSquadWorkout));
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleStayLevel  =  async () => {
             singleSquadWorkout['SingleSquadWorkout'].completedCount += 1;
             await AsyncStorage.setItem('@singleSquadWorkoutStatus', JSON.stringify(singleSquadWorkout));
             handleResetWorkout();
  ...
  const handleLevelUp =  async () => {
            singleSquadWorkout['SingleSquadWorkout'].completedCount += 1;
            singleSquadWorkout['SingleSquadWorkout'].level = level + 1;
            await AsyncStorage.setItem('@singleSquadWorkoutStatus', JSON.stringify(singleSquadWorkout));
            setLevel(level + 1);
            handleResetWorkout();
            setModalVisible(false);
...
export default SingleSquadWorkoutScreen;
