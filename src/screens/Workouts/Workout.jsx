...
const WorkoutScreen = ({ navigation }) => {
  ...
  const [exerciseReps, setExerciseReps] = useState(
    ExerciseService.increaseRepsByLevel(exercises[0], level)
  );
  ...
  useEffect(() => {
    const newReps = ExerciseService.increaseRepsByLevel(
  ...
  const getDataFromAsyncStorage = async () => {
    try {
      ...
      if (storedStatus === null) {
        ...
        const currentLevel =
          fullBodyWorkoutStatusData["HomeFullBodyWorkout"]?.level || 1;
  ...
  const handleCompleteSet = async () => {
    ...
    const completedExerciseStats = fullBodyWorkout[completedExerciseName] || {
      completedCount: 0,
      totalReps: 0,
    };
    ...
    try {
      await AsyncStorage.setItem(
        "@fullBodyWorkoutStatus",
        JSON.stringify(fullBodyWorkout)
      );
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <>
      ...
      <LinearView>
          ...
          {exerciseReps.map((rep, index) => (
            ...
            >
              <Text
                style={[
                  styles.setText,
                  currentSet === index + 1 && styles.activeSetText,
                ]}
              >
                {rep}
              </Text>
        ...
        <LottieView
          source={animations[exercise.image]}
          autoPlay
          loop
          style={styles.image}
        />
        ...
        {isResting && (
          <Text style={styles.restTimeText}>Rest Time: {restTime}</Text>
        )}

        <View style={styles.buttonContainer}>
          {!isResting &&
            (exerciseIndex < exercises.length - 1 ||
            currentSet < exercises[exerciseIndex].sets ? (
              <DoneButton
                title={t("CompleteSet")}
                fontSize={17}
