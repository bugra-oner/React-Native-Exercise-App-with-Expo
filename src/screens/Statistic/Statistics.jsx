...
const StatisticsScreen = ({ navigation }) => {
  ...
  const getStatistics = async () => {
    try {
      const fullBodyStats = await AsyncStorage.getItem(
        "@fullBodyWorkoutStatus"
      );
      const upperBodyStats = await AsyncStorage.getItem(
        "@upperBodyWorkoutStatus"
      );
      const lowerBodyStats = await AsyncStorage.getItem(
        "@LowerBodyWorkoutStatus"
      );
      ...
      const squadStats = await AsyncStorage.getItem(
        "@singleSquadWorkoutStatus"
      );
      if (squadStats) {
      ...
      const tricepsStats = await AsyncStorage.getItem(
        "@singleTricepsWorkoutStatus"
      );
      if (tricepsStats) {
      ...
      const pushUpsStats = await AsyncStorage.getItem(
        "@singlePushUpWorkoutStatus"
      );
      ...
      const sitUpsStats = await AsyncStorage.getItem(
        "@singleSitUpsWorkoutStatus"
      );
      if (sitUpsStats) {
  ...
  return (
    <>
      ...
      <ScrollView style={styles.container}>
        ...
        <View>
          <Text style={styles.Header}>{t("SingleExercises")}</Text>
          ...
          <View style={styles.bottomView}>
            <CreaterCard marginTop="6%" height={140} />
          </View>
        </View>
      </ScrollView>
    </>
